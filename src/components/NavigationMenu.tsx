...
const navigationItems = [
  ...
  { href: '/phone-number-qr-code', label: 'Phone QR Code',
    icon: (
      <svg className={iconClass} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}>
  ...
  { href: '/email-qr-code-generator', label: 'Email QR Code',
    icon: (
      <svg className={iconClass} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}>
...
export function NavigationMenu({ isOpen, onClose }: NavigationMenuProps) {
  ...
  return (
    <>
      ...
      {/* Menu */}
      <div ref={menuRef} className="fixed top-0 left-0 h-full w-80 bg-white dark:bg-slate-900 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out rounded-r-3xl border-r border-gray-200/50 dark:border-slate-800/50" role="dialog" aria-modal="true" aria-label="Main menu">
